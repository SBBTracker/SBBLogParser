...
if os_name == 'Linux':
    sbb_root = Path(expanduser(
        "~/.steam/steam/steamapps/compatdata/1367020/pfx/drive_c/users/steamuser/AppData/LocalLow/Good Luck Games/Storybook Brawl"))
elif os_name == "Windows":
    sbb_root = Path(os.environ["LOCALAPPDATA"]).parent.joinpath("LocalLow/Good Luck Games/Storybook Brawl")
elif os_name == "Darwin":
    raise OSError  # todo: implement Mac
else:
    raise OSError
...
class Update:
    ...
    def to_json(self):
        return json.dumps({'job': self.job, 'state': self.state.to_json()})
